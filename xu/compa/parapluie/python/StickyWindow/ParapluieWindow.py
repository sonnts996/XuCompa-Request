...
import xu.compa.parapluie.python.StickyWindow as StickyWindow
from xu.compa.parapluie.python.Parapluie.ParapluieDefine import *


class ParapluieWindow(QMainWindow):
    resized = pyqtSignal()
    closed = pyqtSignal()
    ...
    def closeEvent(self, a0: QtGui.QCloseEvent):
        self.closed.emit()
        super(ParapluieWindow, self).closeEvent(a0)
    ...
    def addAlert(self, new: StickyWindow.Alert):
    ...
    def refreshAlert(self):
        ...
        for i2 in range(len(showing)):
            a2: StickyWindow.Alert = showing[i2]
            if i2 == 0:
                a2.enableFollowParentRelative()
                a2.setAnchor(Right, Top, x, y)
            else:
                prev: StickyWindow.Alert = showing[i2 - 1]
                y = y + prev.height() + 10
                a2.enableFollowParentRelative()
                a2.setAnchor(Right, Top, x, y)
    ...
    def onAlertClose(self, a: StickyWindow.Alert):
        # inner destroy
